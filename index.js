...
const port = 3000 // 2- todo local, arquivo tem uma porta de acesso, este é meu acesso ao node. (de 3k pra cima)
...
const checkuserId = (request, response, next) =>{  // 31- eu vou chegar todos usuarios se são validos de uma só vez asim o ex:18 não precisaria estar em todas APPS 
...
app.get('/users', (request, response) =>{  // 5- os APP são rotas o .GET vai mostrar o que tem na porta que acessei e é usado 2 parametros 1 é a rota que usei o USERS e o outro é uma função, na qual usei o REQUEST eo RESPONSE que é o que o express pede
    return response.json(users)  // 6- o .JSON é pra me retornar no meu front end o que tem no meu USERS
})


app.post('/users', (request, response) =>{  // 8- os APP são rotas o .POST vai criar informações no back-end é usado 2 parametros 1 é a rota que usei o USERS e o outro é uma função, na qual usei o REQUEST eo RESPONSE que é o que o express pede
    const { name, age } = request.body      // 10- vou usar o request.BODY para criar meus dados

    const user = { id:uuid.v4(), name, age }        // 12- criando o .V4() ele cria um id unico P/ cada cliente ex: um cpf
...
app.put('/users/:id',checkuserId, (request, response) =>{  // 13- os APP são rotas o .PUT vai acessar os dados do cliente e é usado 2 parametros 1 é a rota que usei o USERS e o outro é uma função, na qual usei o REQUEST eo RESPONSE que é o que o express pede
    ...
    const {name, age} = request.body   // 16- aqui eu vo busca o nome e idade do cliente pelo .BODY
    const index = request.userIndex // 33- aqui meu index"clliente" é o resultado do .userIndex que é a verificação de cada cliente
    ...
    const updatedUser = {id, name, age}  // 17- vou montar meu usuario criando updat.., usando o id dele com nome e idade
...
app.delete('/users/:id',checkuserId , (request, response) =>{  // 23- o .DELETE vai deletar o "ID" e é usado 2 parametros 1 é a rota que usei o USERS e o outro é uma função, na qual usei o REQUEST eo RESPONSE que é o que o express pede
